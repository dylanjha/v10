...
- The provider lazy-loads the pack for the resolved locale automatically and keeps it in a layer above the shared registry (<DocsLink slug="reference/register-i18n">`registerI18n`</DocsLink>). Translations merge along a fallback chain — `es-419` falls back to `es`, then `en`. See <DocsLink slug="concepts/i18n">Internationalization</DocsLink> for the full translation model and merge order.
...
```html
<script type="module" src="https://cdn.jsdelivr.net/npm/@videojs/html@10.0.0-beta.26/cdn/locales/es.js"></script>
...
import { registerI18n } from 'https://cdn.jsdelivr.net/npm/@videojs/html@10.0.0-beta.26/cdn/i18n.js';
