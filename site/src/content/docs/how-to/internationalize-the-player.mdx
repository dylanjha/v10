...
category: 'capability'
api: ['I18nProvider', 'registerI18n', 'useTranslator', 'loadLocale']
keywords:
  [
    'video player translation',
    'i18n',
    'locale',
    'translated labels',
    'player language',
...
- Locale packs load lazily through `loadLocale`; the provider does this for the resolved locale automatically. Packs register into a shared registry (<DocsLink slug="reference/register-i18n">`registerI18n`</DocsLink>), merged along a fallback chain — `es-419` falls back to `es`, then `en`. See <DocsLink slug="concepts/i18n">Internationalization</DocsLink> for the full translation model and merge order.
- `registerI18n(locale, translations)` **merges**: each call adds or replaces keys for that locale without wiping prior registrations, and later registrations win for the same key. Overriding a handful of strings is the same call as registering a full locale.
...
- Translation keys are namespaced strings (`buttons.play`, `errors.network`, `menu.quality`); the full list lives at <DocsLink slug="reference/translation-phrases">Translation keys</DocsLink> and is typed, so typos fail in TypeScript.
...
Only the keys you pass change; the rest of the `ja` pack stays intact.
...
CDN locale modules call `registerI18n` on import:
...
For a custom locale, import `registerI18n` from the shared `cdn/i18n.js` bundle (so every chunk shares one registry) and load your module after the player script:
