...
api: ['usePlayer', 'PlayerController']
keywords: ['custom video player controls', 'skip intro button', 'custom player component', 'player state and actions']
---
...
<TabsRoot client:idle>
  ...
  <TabsPanel client:idle value="component" initial>
    ...
    // Match the features your player uses so state and actions are typed
...
Access state and actions with the <DocsLink slug="reference/use-player">`usePlayer`</DocsLink> hook that <DocsLink slug="reference/create-player">`createPlayer`</DocsLink> returns. It knows which features your player has, so state and actions are typed. (The standalone `usePlayer` export from `@videojs/react` returns an untyped store, so its values are `unknown` in TypeScript.)
...
You're using the standalone `usePlayer` export or the shared `playerContext`, which don't know which features your player has. Use the `usePlayer` hook (React) or the typed `context` (HTML) returned by `createPlayer` with your player's features.

### The component renders but never updates

In React, the component sits outside `<Player.Provider>`, so `usePlayer` has no store to subscribe to. In HTML, the element sits outside `<video-player>`, or the class doesn't extend `MediaElement`, so `PlayerController` has nothing to schedule updates on.
