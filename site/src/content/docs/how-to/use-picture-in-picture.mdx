...
- `pipAvailability` is `'available'`, `'unavailable'`, or `'unsupported'`. It starts as `'unavailable'`, then settles to `'available'` or `'unsupported'` when the feature attaches to a media element.
- `requestPictureInPicture()`, `exitPictureInPicture()`, and `togglePictureInPicture()` change the presentation. Their promises reject when the browser blocks the request; in a browser without the picture-in-picture API they resolve without changing anything.
