...
**Anything the player can do gets a capability guide.** Write a concept page when the reader needs understanding that spans multiple capabilities (features, skins, security). Use this table to decide between the two:

| Concept page | How-to guide |
|---|---|
| Understanding that spans capabilities | One capability or one build |
...
A capability earns its own guide when it has a distinct developer intent, real browser or platform constraints, several APIs that need explaining together, and strong search value. Otherwise it stays a section of a broader guide (volume lives in Playback; chapters live in Text Tracks).
...
A guide may skip a section that doesn't apply, but the sections it has keep these names and this order. One capability per page, with a literal, searchable title (Autoplay, not Advanced Playback) — one page should fully answer one capability question for a reader or a coding agent.
...
Capability guides also carry machine-readable discovery fields so agents and search can find them:
